...
# ── Numeric configuration is clamped, not trusted ────────────────────────────
#
# Same class as the vm-watchdog finding on PR #15: invalid configuration must
# not be taken literally. `or <default>` covers absent and zero but not
# negative — parse_duration_to_seconds accepts "-5s" on purpose, because
# west-of-UTC broker offsets need the sign.
#
# Clamped rather than refused: this module is imported by the whole API, so
# raising on an optional feature's tuning value would stop trading too.

from mt5api.config import _chartctl_bytes, _chartctl_seconds


@pytest.mark.parametrize("raw", ["-5s", "-1h", "-90m"])
def test_a_negative_duration_is_clamped_to_the_floor(raw):
    """A negative stale window calls every observation stale; a negative hint
    interval is permanently due. Both look like a broken loader."""
    assert _chartctl_seconds(raw, "60s", 1) >= 1


@pytest.mark.parametrize("raw", [None, "", 0])
def test_absent_and_zero_fall_back_to_the_default(raw):
    assert _chartctl_seconds(raw, "60s", 1) == 60


@pytest.mark.parametrize("raw,expected", [("12s", 12), ("2m", 120), ("1h", 3600)])
def test_valid_durations_are_left_alone(raw, expected):
    """The clamp must not quietly rewrite a configuration someone meant."""
    assert _chartctl_seconds(raw, "60s", 1) == expected


def test_a_negative_upload_cap_cannot_reject_every_file():
    """read(cap + 1) with cap = -1 reads nothing, then len(0) > -1 rejects the
    upload while reporting a limit of -1 bytes."""
    assert _chartctl_bytes(-1, 16 * 1024 * 1024, 1024) >= 1024


def test_absent_upload_cap_uses_the_default():
    assert _chartctl_bytes(None, 16 * 1024 * 1024, 1024) == 16 * 1024 * 1024


def test_a_valid_upload_cap_is_left_alone():
    assert _chartctl_bytes(32 * 1024 * 1024, 16 * 1024 * 1024, 1024) == 32 * 1024 * 1024
