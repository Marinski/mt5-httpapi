...
def _vm_group_filter():
    """Return the set of (broker, account, instance) tuples this VM hosts, or
    None when there is nothing to filter by.

    Mirrors the awk port-filter in scripts/healthcheck.sh (the container-side
    healthcheck): reads the per-VM group file docker-compose bind-mounts at
    config/vm-group.txt — generated by `config_helper.py vm_group <vm>` — and
    parses `broker account [instance]` lines. A missing file, or one with no
    valid entries, means "no filter", not "select nothing": that is exactly
    what keeps a single-VM install probing every port, and it matches the
    empty-file fallback the awk program documents. check_health.py imports
    this hook so the host-side start.bat status loop applies the same per-VM
    scope as the container healthcheck.
    """
    group_path = os.path.join(_SHARED_DIR, "config", "vm-group.txt")
    try:
    ...
    for line in lines:
        ...
        if len(fields) >= 2:
            instance = fields[2] if len(fields) >= 3 else DEFAULT_INSTANCE
            allowed.add((fields[0], fields[1], instance))
    return allowed or None


def _in_group(terminal, allowed):
    """True when terminal is in this VM's group, or there is no group at all.

    `allowed` is whatever _vm_group_filter() returned: None means "no group
    file / no valid entries", so every terminal is in scope. Otherwise the
    terminal's broker/account/instance is compared against the parsed group
    keys, with a missing instance normalized to DEFAULT_INSTANCE the same way
    the `vm_group` writer does, so the two sides always agree.
    """
    if allowed is None:
        return True
    instance = _normalize_instance(terminal.get("instance"))
    return (terminal["broker"], terminal["account"], instance) in allowed
