...
# Placeholder rather than "${AUTH[@]}": this line is printed, not run, and
# expanding the array here would echo the bearer token to the terminal (and
# into whatever captures this output).
echo "  curl -X DELETE -H \"Authorization: Bearer \$MT5_API_TOKEN\" \"$MT5_API_URL/deployments/$DEPLOY_ID\""
